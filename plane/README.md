...
- `docker-compose.yaml`
- `plane.env` and `plane-secrets.env`
...
In case you want to make changes to `plane.env` and `plane-secrets.env` variables, we suggest you to stop the services before doing that.
...
In case you want to make changes to `plane.env` and `plane-secrets.env` variables, without stopping the server or you noticed some abnormalies in services, you can restart the services with `RESTART` / `REDEPLOY` option.
...
By choosing this, it will stop the services and then will download the latest `docker-compose.yaml` and `plane.env` and `plane-secrets.env`.
...
> It is very important for you to validate the `plane.env` and `plane-secrets.env` for the new changes.

Once done with making changes in `plane.env` and `plane-secrets.env` file, jump on to `Start Server`
...
Once done with making changes in `plane.env` and `plane-secrets.env` file, jump on to `Redeploy Stack`
