...
DOCKER_SECRETS_PATH=$PLANE_INSTALL_DIR/plane-secrets.env
...
function syncEnvFile(){
    ...
    if [ -f "$PLANE_INSTALL_DIR/plane-secrets.env.bak" ]; then
        while IFS= read -r line
        do
            # ignore is the line is empty or starts with #
            if [ -z "$line" ] || [[ $line == \#* ]]; then
            ...
            key=$(echo "$line" | cut -d'=' -f1)
            value=$(getEnvValue "$key" "$PLANE_INSTALL_DIR/plane-secrets.env.bak")
            if [ -n "$value" ]; then
                updateEnvFile "$key" "$value" "$DOCKER_SECRETS_PATH"
            fi
        done < "$DOCKER_SECRETS_PATH"
    fi
...
function download() {
    ...
    RESPONSE=$(curl -sSL -H 'Cache-Control: no-cache, no-store' -w "HTTPSTATUS:%{http_code}" "$RELEASE_DOWNLOAD_URL/$APP_RELEASE/secrets.env?$(date +%s)")
    BODY=$(echo "$RESPONSE" | sed -e 's/HTTPSTATUS\:.*//g')
    STATUS=$(echo "$RESPONSE" | tr -d '\n' | sed -e 's/.*HTTPSTATUS://')

    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $PLANE_INSTALL_DIR/secrets-upgrade.env
    else
        # Fallback to download from the raw github url
        RESPONSE=$(curl -sSL -H 'Cache-Control: no-cache, no-store' -w "HTTPSTATUS:%{http_code}" "$FALLBACK_DOWNLOAD_URL/secrets.env?$(date +%s)")
        BODY=$(echo "$RESPONSE" | sed -e 's/HTTPSTATUS\:.*//g')
        STATUS=$(echo "$RESPONSE" | tr -d '\n' | sed -e 's/.*HTTPSTATUS://')
        ...
        if [ "$STATUS" -eq 200 ]; then
            ...
            echo "Failed to download secrets.env. HTTP Status: $STATUS"
            echo "URL: $RELEASE_DOWNLOAD_URL/$APP_RELEASE/secrets.env"
            mv $PLANE_INSTALL_DIR/archive/$TS.docker-compose.yaml $PLANE_INSTALL_DIR/docker-compose.yaml
            exit 1
        fi
    fi
    ...
    if [ -f "$DOCKER_SECRETS_PATH" ];
    then
        cp "$DOCKER_SECRETS_PATH" "$PLANE_INSTALL_DIR/archive/$TS.secrets.env"
        cp "$DOCKER_SECRETS_PATH" "$PLANE_INSTALL_DIR/plane-secrets.env.bak"
    fi

    mv $PLANE_INSTALL_DIR/variables-upgrade.env $DOCKER_ENV_PATH
    mv $PLANE_INSTALL_DIR/secrets-upgrade.env $DOCKER_SECRETS_PATH
    ...
    if [ "$LOCAL_BUILD" == "true" ]; then
        ...
        CUSTOM_BUILD="false"
        updateCustomVariables
        /bin/bash -c "$COMPOSE_CMD -f $DOCKER_FILE_PATH --env-file=$DOCKER_ENV_PATH --env-file=$DOCKER_SECRETS_PATH pull --policy always"
...
function startServices() {
    /bin/bash -c "$COMPOSE_CMD -f $DOCKER_FILE_PATH --env-file=$DOCKER_ENV_PATH --env-file=$DOCKER_SECRETS_PATH up -d --pull if_not_present --quiet-pull"
...
function stopServices() {
    /bin/bash -c "$COMPOSE_CMD -f $DOCKER_FILE_PATH --env-file=$DOCKER_ENV_PATH --env-file=$DOCKER_SECRETS_PATH down"
}
